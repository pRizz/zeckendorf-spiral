...
export const FibonacciCanvas = forwardRef<FibonacciCanvasRef, FibonacciCanvasProps>(
  ...
  const { showLabels, useSqrtMode, theme, lineThickness } = useSettings();
  ...
  const draw = useCallback((currentCount: number) => {
    ...
    if (floorCount === ceilCount || progress === 0) {
      drawFibonacciEvenIndexSquaresAnimated(canvas, floorCount, floorCount, 1, colors, padding, showLabels, useSqrtMode, lineThickness);
    } else {
      // Animate between floor and ceil
      drawFibonacciEvenIndexSquaresAnimated(canvas, floorCount, ceilCount, progress, colors, padding, showLabels, useSqrtMode, lineThickness);
    }
  }, [setupCanvas, getColors, showLabels, useSqrtMode, lineThickness]);
  ...
  useEffect(() => {
    ...
    if (isDiscreteStep) {
      ...
      const tick = (now: number) => {
        ...
        drawFibonacciEvenIndexSquaresAnimated(canvas, from, to, eased, colors, padding, showLabels, useSqrtMode, lineThickness);
  ...
  }, [count, draw, setupCanvas, getColors, showLabels, useSqrtMode, lineThickness]);

  // Redraw when settings change
  useEffect(() => {
    draw(count);
  }, [showLabels, useSqrtMode, theme, lineThickness, draw, count]);
